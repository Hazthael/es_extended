...
ESX.SavePlayer = function(xPlayer, cb)
	...
	table.insert(asyncTasks, function(cb)
		MySQL.Async.execute('UPDATE users SET job = @job, job_grade = @job_grade,org = @org, gradeorg = @org_gradeorg, loadout = @loadout, position = @position WHERE identifier = @identifier', {
			['@job']        = xPlayer.job.name,
			['@job_grade']  = xPlayer.job.grade,
			['@org']        = xPlayer.org.name,
			['@gradeorg']  = xPlayer.org.gradeorg,
...
ESX.DoesOrgExist = function(org, grade)
	grade = tostring(org_gradeorg)

	if org and grade then
		if ESX.Jobs[org] and ESX.Jobs[org].grades[org_gradeorg] then
			return true
		end
	end
